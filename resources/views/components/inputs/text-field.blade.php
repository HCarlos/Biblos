...
@if(is_null($cols1))
    <div class="col-sm-{{$cols}}">
        ...
        <input type="{{$tipo}}" class="form-control {{ $Nombre }} {{$class ? $class : ''}} has-error form-error " placeholder="{{ $nombrees }}" name="{{ $Nombre }}" id="{{ $Nombre }}" value="{{$valor}}" {{$deshabilitado}} {{$sololectura}} >
