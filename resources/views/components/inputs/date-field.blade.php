...
<div class="col-sm-{{$cols}}">
    ...
    <input type="date" class="form-control {{ $Nombre }} {{$class ? $class : ''}} has-error form-error " placeholder="{{ ucwords($nombre) }}" name="{{ $Nombre }}" id="{{ $Nombre }}" value="{{$valor}}" {{$deshabilitado}} min="1900-01-01" max="{{ now() }}">
